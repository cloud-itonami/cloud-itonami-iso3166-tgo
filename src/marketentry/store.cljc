(ns marketentry.store
  "SSoT for the Togo (TGO) market-entry compliance actor, behind a
...
(defn demo-data
  ...
  `:reserved-for-youth-women?` / `:procedure-type` are ground truth for
  the `youth-women-procedure-mismatch` flagship check (Décret
  n°2018-028/PR Art. 2's permitted-simplified-procedures restriction);
  `:requires-nif?` / `:nif-verified?` are ground truth for the
  conditional NIF check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Lomé Artisans Textiles SARL" :portal "osmapt.arcop.tg"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :reserved-for-youth-women? true :procedure-type :demande-de-cotation
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "TGO" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "osmapt.arcop.tg"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :reserved-for-youth-women? true :procedure-type :demande-de-cotation
             :requires-nif? true :nif-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Kara Bâtiment SA" :portal "osmapt.arcop.tg"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "TGO" :status :intake}
    "eng-4" {:id "eng-4" :operator "Sokodé Jeunes Entrepreneurs Coopérative" :portal "osmapt.arcop.tg"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :reserved-for-youth-women? true :procedure-type :appel-offres-ouvert
             :requires-nif? true :nif-verified? true
             :drafted? false :submitted? false
             :jurisdiction "TGO" :status :intake}
    "eng-5" {:id "eng-5" :operator "Kpalimé Logistics SARL" :portal "osmapt.arcop.tg"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :reserved-for-youth-women? false :procedure-type :appel-offres-ouvert
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               reserved-for-youth-women? procedure-type
  ...
  (cond-> {:engagement/id id}
    ...
    (some? reserved-for-youth-women?)     (assoc :engagement/reserved-for-youth-women? reserved-for-youth-women?)
    procedure-type                        (assoc :engagement/procedure-type (enc procedure-type))
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/reserved-for-youth-women? :engagement/procedure-type
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :reserved-for-youth-women? (boolean (:engagement/reserved-for-youth-women? m))
     :procedure-type (dec* (:engagement/procedure-type m))
