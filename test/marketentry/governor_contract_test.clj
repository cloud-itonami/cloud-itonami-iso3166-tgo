  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Lomé Artisans Textiles SARL"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Lomé Artisans Textiles SARL" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest youth-women-procedure-mismatch-is-held-and-unoverridable
  (testing "a reserved-for-youth-women? engagement whose declared procedure type isn't Art. 2's permitted set -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:youth-women-procedure-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest nif-unverified-is-held-and-unoverridable
  (testing "unverified NIF (Numéro d'Identification Fiscale) when required -> HARD hold"
    (let [[db actor] (fresh)
